...
export class ProjectsFilter extends Component {
    ...
    updateFilteredProjects = () => {
        ...
        const { selectedSkill, selectedCategory, sortMethod } = this.state

        const filteredProjects = Object.values(projects).filter(p => {
            const isInCategory =
                selectedCategory !== 0 ? p.category.includes(selectedCategory) : true
    ...
    render() {
        ...
        const SkillsItems = Object.values(skills)
            .sort((a, b) => {
                return a.name.localeCompare(b.name)
            })
            .map(s => {
        ...
        return (
            <ProjectsFilterWrapper>
                ...
                <Row>
                    <FilterIcon /> {CategoryDropdown} <AlignToDropdown>Or</AlignToDropdown>{' '}
                    {SkillsDropdown}
                </Row>
            </ProjectsFilterWrapper>
        )
